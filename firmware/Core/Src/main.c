...
int main(void) {
    ...
    // Set NVIC priority grouping so that all interrupt priority bits
    // are treated as preemption priority bits (no subpriority bits).
    // FreeRTOS uses BASEPRI to mask interrupts below
    // configMAX_SYSCALL_INTERRUPT_PRIORITY during FromISR API calls.
    // If the grouping is wrong, BASEPRI is not applied and accessing
    // kernel objects (like queues) from an ISR can cause BusFaults.
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef* htim) {
    ...
    if (htim->Instance == TIM13) {
        ...
        button_debounce_timer_callback(htim);
