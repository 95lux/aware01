...
y[0] = 1
lp_state = 0.0
d = 0.2 # lowpass coefficient (0 < d < 1, smaller = more lowpass)

for n in range(N):
    if n >= D:
        feedback = alpha * lp_state
        lp_state = (1 - d) * y[n - D] + d * lp_state
    else:
        feedback = 0.0
    y[n] = x[n] + feedback
...
ax1.set_xlim(-1, 6*D + 0.5)
ax1.set_title("Impulse Response (Time Domain)")
ax1.set_ylabel("h[n]")

# frequency response - multiple alphas with lowpass
alphas = [0.3, 0.7, 0.90]
linestyles = ['-', '--', ':']
for a, ls in zip(alphas, linestyles):
    ...
    for n in range(N):
        if n >= D:
            lp_state_a = (1 - d) * y_a[n - D] + d * lp_state_a
            feedback = a * lp_state_a
        else:
            feedback = 0.0
        y_a[n] = x[n] + feedback
...
plt.savefig(script_dir / "../images/schroeder_reverb/fig_comb_filter_lp_feedback_response.pdf", bbox_inches="tight")
plt.savefig(script_dir / "output/fig_comb_filter_lp_feedback_response.png", bbox_inches="tight")

plt.show()

print("Saved: comb_filter_lp_feedback_response.pdf / comb_filter_lp_feedback_response.png")