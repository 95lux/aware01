...
The design is organised into five functional domains: power delivery, the \ac{MCU}, user interface, control interface, and audio interface.
...
A subsequent \textit{LM1117MP-3.3}\cite{Lm1117} \ac{LDO} regulator provides the \qty{3.3}{V} rail required by the MCU and audio codec. 
...
A \qty{1.27}{mm} pitch \ac{SWD} connector provides debugging and flashing, following the standard STDC14 ARM10 pinout \cite{stmicroelectronicsSTLINKV3SETDebuggerProgrammer}.

\paragraph{User Interface}
Four slide potentiometers (\textit{Bourns PTL30-15R1-103B2}\cite{PTLDatasheet}) are read by the MCU ADC and provide parameter control. 
Two push buttons (\textit{SameSky TS02} \cite{Ts02Datasheet}) are connected as GPIO inputs. 
Four WS2812 RGB LEDs, daisy-chained and driven via a PWM signal, provide visual feedback. Because the WS2812 protocol requires \qty{5}{V} logic, a level shifter based on a Schmitt NOT gate (\textit{TI SN74AHCT1G14}~\cite{Sn74ahct1g14}) converts (and inverts) the \qty{3.3}{V} MCU output. 
...
Eurorack control signals are accepted via \qty{3.5}{mm} sockets (\textit{QingPu WQP-PJ398SM} \cite{35mmJackVertical}): 
...
The CV and V/Oct signals pass through opamp (\textit{TI OPA2322} \cite{Opa2322}) attenuation stages before being read by the MCU ADC.

\paragraph{Audio Interface}
Stereo audio input and output are provided via pairs of \qty{3.5}{mm} mono sockets (\textit{QingPu WQP-PJ398SM} \cite{35mmJackVertical}) at Eurorack signal levels. 
...
Configuration of the codec is done via I\textsuperscript{2}C.
