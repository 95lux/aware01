...
In this project, a $Q48.16$ phase accumulator is used to track the playhead position. 
This phase is passed into the \texttt{hermite\_interpolate} function \ref{lst:hermite-interpolate} as \mintinline{c}|uint32_t phase|. The position is stored in a single \texttt{uint64\_t} variable, allowing it to be read and written in a single CPU operation. The 48 most significant bits encode the integer part of the position, while the 16 least significant bits represent the fractional part. 
...
While a fixed-point phase accumulator provides bit-exact and deterministic phase tracking, for short playback durations, the advantages are minimal. 
Single-precision floating-point arithmetic is good enough to represent fractional sample positions without noticeable phase drift. 
So, the fixed-point implementation can be seen as somewhat cautious or too complicated for the current application. 
However, it provides a strong and flexible framework that would remain accurate even for long audio playback, looping oscillators, or systems without hardware floating-point support.