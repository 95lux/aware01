...
TIM13 and TIM14 operate in one-pulse mode (OPM), generating a single pulse per trigger event rather than running continuously.
...
TIM17's period-elapsed ISR issues a direct-to-task notification to \texttt{UserInterfaceTask}, keeping all animation logic within the task context.
...
All gate and button inputs generate falling-edge interrupts at NVIC priority~10 (see Table~\ref{tab:exti}), permitting FreeRTOS API calls (\texttt{xTaskNotifyFromISR}) directly inside these handlers.
...
The I2S master clock (MCK) on PC4 is driven by PLL2-P at \SI{12.288}{\mega\hertz}, exactly $256 \times f_s$ at \SI{48}{\kilo\hertz}, allowing the codec to derive all internal clocks from this single external reference without its own PLL.
...
Buffer sizes for the tape buffers (Section~\ref{sec:tapep-buffers}) and reverb delay lines (Section~\ref{sec:schroeder_memory_allocation}) are tuned to maximise SRAM utilisation: 
...
The AudioTask initialises the codec and audio engine, then enters a perpetual processing loop. 
...
Clock dividers NDAC and MDAC are selected at runtime based on the requested sample rate according to $f_s = f_\text{MCLK} / (N \cdot M \cdot \text{OSR})$, with a \qty{12.288}{MHz} master clock and fixed OSR of 128. 
