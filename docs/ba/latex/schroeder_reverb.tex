...
by pushing parameters to extremes, the effect can produce sounds ranging from harsh metallic resonances to wide, lush, near-infinite spatial decay -- far beyond conventional room simulation.
...
For this reason, one of the earliest concepts of artificial reverberation based on discrete-time signal processing was chosen -- constructed by Schröder in the early 1960s and known as the \textit{Schröder Reverberator}.

\subsection{Theory of the Schröder Reverb}
% Schroeder structure, why prime delays, what RT60 is.

Schröders original proposal is a combination of \textit{comb} and \textit{allpass} filters. \cite{m.r.schroederJournalAudioEngineering1962}[pp.~219-223]
...
Figure \ref{fig:schroeder_block} shows the grobe structure of the implemented reverb.
...
At the basis of the \textit{Schröder Reverberator} is a structure of parallel comb filters.
...
\begin{wrapfigure}{r}{0.54\textwidth}
	\centering
	\includegraphics[width=0.52\textwidth]{../images/schroeder_reverb/fig_comb_filter_response.pdf}
...
Instead of using a standard feedback comb filter, this project employs a \textit{lowpass-feedback} comb filter (LBCF), which places a filter $H_f(z)$ in the feedback path alongside the feedback gain $\alpha$.
...
\begin{wrapfigure}{r}{0.50\textwidth}
	\centering
	\vspace{-2.0em}
	\includegraphics[width=0.53\textwidth]{../images/schroeder_reverb/fig_comb_filter_lp_feedback_response.pdf}
...
In the context of the Schröder reverberator, allpass filters are used as \textit{diffusers}: they increase the echo density of the late reverberation without coloring the sound, complementing the tonal decay provided by the comb filter bank. \cite{juliuso.smithPhysicalSignalProcessing}
...
It becomes clear, how the Schröder allpass works. The numerator and denominator of $H(z)$ are mirror images of each other. 
...
\begin{wrapfigure}{r}{0.54\textwidth}
	\centering
	\vspace{-2.0em}
	\includegraphics[width=0.52\textwidth]{../images/schroeder_reverb/fig_allpass_response.pdf}
...
The implemented reverberator follows a simplified variant of the \textit{Freeverb} \cite{freeverb-github} variant of the Schröder structure -- a public-domain C++ program, used extensively in the free-software community -- adapted here for embedded C on a microcontroller target. 
...
\begin{figure}[htbp]
	...
	\vspace{-1.1em}
	\makebox[\textwidth][c]{
		\includegraphics[width=0.9\textwidth]{../images/schroeder_reverb/schroeder_reverb_block_detailed.drawio.pdf}	
		}
	\vspace{-2.0em}
	\caption{Schroeder Reverb Block Diagram}
...
Both underlying \texttt{sr\_delay\_t} data structures advance with a write index \texttt{idx} that advances by one sample each tick and wraps modulo size. 
The delay tap is read from position \mintinline{c}|(idx + size - length) % size| (line 3 for comb, 21 for allpass), placing it exactly \texttt{length} samples behind the write head. 
This avoids any branch on the read side and keeps both operations O(1). 

For the comb filter, the fed-back sample passes through a one-pole lowpass (line 9) before being added to the input and written into the buffer (line 10), implementing the damped feedback loop in-place. 

The allpass filter uses the same ring structure but applies the Schroeder allpass topology directly (lines 22--23): the delayed sample \texttt{buf} is summed with the negated input for the output, while the write combines the input with the unfiltered delayed sample scaled by \texttt{feedback}, preserving the all-pass magnitude response while dispersing phase.
...
\paragraph{Delay Length Selection} Using mutually prime delay line lengths — values sharing no common factors — helps avoid spectral clustering and unnatural resonances in the frequency response. 
...
The delay lengths were selected empirically by ear from prime numbers, starting from a base set and scaled by a room size parameter. The chosen base delay lengths are:
...
\paragraph{Controllable Parameters} The reverb exposes four parameters via a public API:

\begin{description}
	\item[\texttt{wet}] Controls the dry/wet balance, with $\texttt{dry} = 1 - \texttt{wet}$ enforced automatically.
	\item[\texttt{feedback} $(\alpha)$] Scales the RT60 decay time, routed through the stability macros to compensate for room size.
	\item[\texttt{size}] Uniformly scales all delay lengths while preserving their prime relationships. 
	\item[\texttt{lp\_alpha} $(d)$] Controls the cutoff of the one-pole IIR filter in each comb feedback path, attenuating high frequencies and producing a more natural, air-absorbed decay.
\end{description}
...
\end{minted}
This means that calling public API function \texttt{schroeder\_rev\_set\_size} alone changes the effective feedback range, even without a subsequent call to \texttt{schroeder\_rev\_set\_feedback}.

\subsection{Evaluation}

\subsubsection{Impulse Response Decay}

Figure~\ref{fig:schroeder_env_decay_measurement} shows the measured envelope decay 
of the reverb unit under varying feedback and room size parameters. Each curve 
...
for the shorter decay at smaller room sizes --- increasing feedback further risks 
...
\paragraph{LP enabled} With the lowpass active ($d = 0.4$), high-frequency energy 
...
\paragraph{LP disabled} Without lowpass damping, energy is distributed more evenly 
across the frequency spectrum throughout the decay, resulting in a brighter and more 
unnatural sounding tail.

\paragraph{Summary} Both variants have usable applications in music production. 
