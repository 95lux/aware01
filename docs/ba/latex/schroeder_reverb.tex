...
The comb filter runs the feedback sample through a one-pole lowpass (line 9) before writing it back with the input (line 10), implementing damped feedback in-place. The allpass uses the same ring structure but applies the Schroeder topology (lines 22--23) directly, yielding flat magnitude response with phase dispersion.
