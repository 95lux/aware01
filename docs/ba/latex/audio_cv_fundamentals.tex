...
of pitch. The construction of scales has fascinated mathematicians and musicians 
since the time of the Greeks \cite[p.~174]{wheelerScienceSound2014}.

The most fundamental division of pitch in music is the \textit{octave}.
The interval between two notes whose frequencies are in a $2:1$ ratio \cite[p.~89]{wheelerScienceSound2014}. 
...
The \textit{Pythagorean scale} for example, attributed to Pythagoras (~580 B.C.), constructs intervals by stacking perfect fifths (frequency ratio $3:2$) and fourths ($4:3$). 
While this produces very consonant fourths and fifths (musical interval of two tones), it results in poorly tuned thirds (another interval) \cite[p.~176]{wheelerScienceSound2014}.

The difficulty in scale construction arises from the fact that no finite number of perfect fifths ($3:2$) can ever exactly equal a whole number of octaves ($2:1$), since no power of $\frac{3}{2}$ is also a power of 2. 
Twelve perfect fifths overshoot seven octaves by a small amount known as the \textit{Pythagorean comma} \cite[p.~180]{wheelerScienceSound2014}. 
...
In modular synthesizer systems, audio parameters such as pitch are controlled by 
analog DC voltages known as \textit{Control Voltages} (CV). 
This principle was introduced by Robert Moog, who produced the first commercially available 
synthesizer in the 1960s \cite{TechnicalDetailsA100}.

In the case of a voltage-controlled oscillator (VCO), pitch is controlled by an external CV input: an increase of \qty{1}{V} corresponds to an increase of one octave in pitch. 
...
The A-100 system manual by Doepfer \cite{TechnicalDetailsA100} serves as the closest reference, as Doepfer invented the Eurorack format in 1995 and their documentation defines the electrical and signal conventions adopted by the modular synthesis community.