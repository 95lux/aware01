...
A sawtooth wave was used as the test signal since it contains all integer harmonics (even and odd), giving a dense spectrum of overtones for evaluating the FFT. \cite[p.~665]{georgearfkenMathematicalMethodsPhysicists1985}
...
To avoid ADC-induced jitter and inaccuracies, the playback speed was fixed at compile time using a macro in \texttt{project\_config.h}, bypassing the control interface entirely:
...
As playback speed drops below 1.0×, the effective upsampling increases and the interpolator is used more often, making its quality more audible. 
At $0.5\times$ and below, ZOH shows clearly visible aliasing humps between the 
...
The Hermite interpolator produces noticeably less aliasing across all upsampling ratios. 
Even at $0.25\times$, where ZOH produces prominent aliasing humps, the Hermite 
...
The parameter limits were chosen empirically to cover the most musically useful range, that produces the most musically useful results (long, dense decay at the upper end and short, subtle ambience at the lower end) while ensuring the filters remain stable throughout.
...
This setup measures the entire latency chain: gate detection, FreeRTOS task 
...
audio-out timing. Channel 1 (blue) is the audio output signal, Channel 2 (red) 
...
\begin{figure}[H]
	...
	\begin{minipage}{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{../images/signal_tests/gate-in_audio_out_latency_tape_enabled/gate-in_audio-out_latency_tape_enabled_02_max_latency_negated.png}
	...
	\begin{minipage}{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{../images/signal_tests/gate-in_audio_out_latency_tape_enabled/gate-in_audio-out_latency_tape_enabled_07_min_latency_negated.png}
...
The peak-to-peak jitter of \qty{0.460}{ms} is well within the theoretical half-block bound of \qty{1.333}{ms}, confirming that command latency is dominated by the block-based processing, as expected. 
...
The DWT counter is enabled during system initialization and mapped to the FreeRTOS \texttt{portGET\_RUN\_TIME\_COUNTER\_VALUE} macro, which allows the scheduler to sample elapsed cycles on every context switch.
...
The remaining 34--40\% is down to the idle task, which indicates sufficient headroom for the current feature set. 
The control interface and user interface tasks consistently measured at 0\%, as both are notification-driven and spend the vast majority of their time blocked.
...
The Schroeder stereo reverb is the most expensive part of the DSP, accounting for approximately half of the total audio task utilization. 
This is due to multiple allpass and comb filter stages per sample across both stereo channels at the full 48~kHz sample rate.
...
Since reverb tails are perceptually diffuse, the reduced high-frequency content would likely be acceptable in practice, and reduce total audio task utilization to approximately 30--36\% under full DSP load.
...
Overall, the V/Oct calibration provides accurate and consistent pitch tracking across the main operating range, with deviations remaining below the perceptual threshold.