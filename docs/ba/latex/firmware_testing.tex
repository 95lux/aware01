...
A sawtooth was chosen as the test signal because it contains both even and odd harmonics, maximising the number of spectral components visible in the FFT output.
...
All measurements fall comfortably within both the \qty{5}{ms} system latency requirement (Section~\ref{sec:system-requirements}) and the \qty{10}{ms} perceptual upper bound for gesture-to-audio latency in  live performance~\cite[p.~13]{davidwesselProblemsProspectsIntimate2002}, with the theoretical worst-case of \qty{4.0}{ms} never approached in practice. 
