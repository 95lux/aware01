...
\]
The cutoff frequency should be well below the audible range to avoid attenuation or phase shift of low-frequency audio content.
With a coupling capacitor of $C_c = \qty{2.2}{\micro\farad}$ and $R_\text{eq} = \qty{40}{\kilo\ohm}$, the cutoff frequency is
\[
f_c = \frac{1}{2 \pi \cdot \qty{40}{\kilo\ohm} \cdot \qty{2.2}{\micro\farad}}
\approx \qty{1.81}{\hertz},
\]
...
The TLV320AIC codec is supplied with \SI{3.3}{\volt}, from which its internal LDO generates a regulated \SI{1.8}{\volt} analog supply (\(AVDD\)), resulting in a common-mode voltage of:
\[
...
The absolute maximum rating specifies that input pins must remain within \(\qty{-0.3}{\volt}\) to \((AVDD + \qty{0.3}{\volt}) = \SI{2.1}{\volt}\), which is the damage threshold of the device, not the operational input range.
...
As a result, the ADC saturates on full-scale Eurorack signals.

\(R_{16}\) was subsequently revised to \qty{1}{\kilo\ohm} to achieve greater attenuation, yielding
\[
...
A BAT54S dual Schottky diode package clamps the attenuated signal to the codec supply rails: one diode clamps to the codecs \(AVDD = \SI{1.8}{\volt}\) and the other to \SI{0}{\volt}, ensuring that voltage peaks or malformed signals cannot exceed the absolute maximum input rating of the ADC.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{../images/hardware_design/simulation/schem_audio_in_stage_simulation.png}
...
The divider consists of \(R_\text{top} = \qty{10}{\kilo\ohm}\) and \(R_\text{bot} = \qty{1}{\kilo\ohm}\), giving an attenuation factor of
\[
a = \frac{R_\text{bot}}{R_\text{top} + R_\text{bot}} = \frac{1}{11} \approx 0.091.
\]
...
\begin{figure}[H]
	...
	\begin{minipage}{0.55\textwidth}
		\includegraphics[width=\textwidth]{../images/hardware_design/simulation/trans_audio_in_simulation.png}
	\end{minipage}
	\hfill
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{../images/hardware_design/simulation/bode_audio_in_simulation.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{../images/hardware_design/simulation/schem_audio_out_stage_simulation.png}
...
\begin{figure}[H]
	...
	\begin{minipage}{0.65\textwidth}
		\includegraphics[width=\textwidth]{../images/hardware_design/simulation/trans_audio_out_simulation.png}
	\end{minipage}
	\hfill
	\begin{minipage}{0.30\textwidth}
		\centering
		\begin{tabular}{lr}
			\toprule
			\multicolumn{2}{l}{\textit{DAC output (Green)}} \\
			...
			\midrule
			\multicolumn{2}{l}{\textit{Jack output (Red)}} \\
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\textwidth]{../images/hardware_design/simulation/bode_audio_out_simulation.png}
...
The CV input stages use an inverting summing amplifier topology, which differs from the audio input stage in two key ways.
...
Both circuits share the same topology with \(R_{in} = \qty{100}{\kilo\ohm}\), differing only in the bias and feedback resistor values to accommodate their respective input ranges.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.65\textwidth]{../images/hardware_design/simulation/schem_cv_simulation.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.65\textwidth]{../images/hardware_design/simulation/schem_voct_simulation.png}
...
\begin{figure}[H]
	\centering
	\begin{minipage}{0.55\textwidth}
		\includegraphics[width=\textwidth]{../images/hardware_design/simulation/trans_cv_simulation.png}
	\end{minipage}
	\hfill
	\begin{minipage}{0.30\textwidth}
		\centering
...
\begin{figure}[H]
	\centering
	\begin{minipage}{0.55\textwidth}
		\includegraphics[width=\textwidth]{../images/hardware_design/simulation/trans_v_oct_simulation.png}
	\end{minipage}
	\hfill
	\begin{minipage}{0.30\textwidth}
		\centering
